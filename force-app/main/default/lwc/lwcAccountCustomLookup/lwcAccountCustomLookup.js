...
export default class LwcAccountCustomLookup extends LightningElement {
 ...
 retrieveAccounts ({error,data}){
     ...
     if(data){
         console.log('data## ' + data.length); //checking the data present or not
         if(data.length>0 && this.isShowResult){ //if data is present 
            this.accountList =data;
            this.showSearchedValues=true;
            this.messageResult=false;
         }
         else if(data.length == 0){ //if data is not 
            this.accountList=[];
            this.showSearchedValues=false;
            if(this.accountName != ''){ //account name is present then messageResult is true
